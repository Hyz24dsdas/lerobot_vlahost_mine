...
def send_next_action_chunk(
    ...
    if chunk.shape[1] == len(ordered_keys):
        ...
        threshold_deg = 50.0/180*3.14159  # 5 度 换成弧度
    ...
    # 记录 action chunk 到文件
    chunk_dir = Path("/home/zzx23457/lerobot_vlahost-main/test_chunk")
    ...
    sent_actions = robot.send_action_chunk(processed_actions)
    ctx.runtime.active_chunk_actions = list(sent_actions)
    ctx.runtime.active_chunk_started_at = time.perf_counter()
    print("action_chunk 长度:", len(processed_actions))
    # Return the last action for telemetry/logging.
    return sent_actions[-1] if sent_actions else None
