...
class RuntimeContext:
    ...
    # Populated only by chunk dispatch. Recording strategies use this schedule
    # to label each 30 Hz observation with the waypoint being executed.
    active_chunk_actions: list[dict] = field(default_factory=list)
    active_chunk_started_at: float | None = None
...
def build_rollout_context(
    ...
    if isinstance(cfg.strategy, EpisodicStrategyConfig) and cfg.strategy.include_velocity:
        state_feature = dataset_features.get("observation.state")
        if state_feature is None:
            raise ValueError("include_velocity requires observation.state")
        dataset_features["observation.velocity"] = {
            "dtype": "float32",
