...
class EpisodicStrategyConfig(RolloutStrategyConfig):
    ...
    # Evaluation lifecycle extension: on shutdown, keep the in-progress
    # LeRobot episode open while commanding and recording a known Home pose.
    # Disabled by default so ordinary episodic recording is unchanged.
    record_home_on_shutdown: bool = False
    shutdown_home_action: list[float] | None = None
    home_tolerance_rad: float = 0.12
    home_timeout_s: float = 12.0
    home_stable_frames: int = 3
