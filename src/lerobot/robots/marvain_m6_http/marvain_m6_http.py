...
class MarvainM6HttpRobot(Robot):
    ...
    server provides endpoints for:
      - GET /state: Returns {
          ...
          "gripper_left": [raw gripper feedback],
          "gripper_right": [raw gripper feedback],
          "quad_image": {"format": "jpeg", "data": "base64..."}
        }
      - POST /action: Accepts {
          "joints": [14 floats in radians],
          "gripper_left": [raw gripper command],
          "gripper_right": [raw gripper command]
        }

    Gripper values:
      - Feedback values from GET /state are passed to observations unchanged
      - Action values are sent to POST /action unchanged
    ...
    def _extract_gripper_pos(value) -> float | None:
        """Extract the raw gripper position from a /state gripper field.
    ...
    def _split_quad_image(quad_image: np.ndarray) -> dict[str, np.ndarray]:
        """Split composite image into named camera views.

        支持两种布局自动推断：
        - 新布局（1+2 堆叠）：顶部一张全宽大图，底部左右两张小图。
        高度比例 2:1，底部均分。顶部 resize 后复制，产出 4 个 view。
        - 旧布局（2x2 quad）：均分成 4 个等大的 cell。

        Returns:
            Dict mapping camera names to image arrays.
        """
        h, w = quad_image.shape[:2]

        # ========== 新布局：1+2 堆叠 ==========
        # 动态条件：高能被 3 整除（2:1 比例），宽能被 2 整除（底部左右均分）
        if h % 3 == 0 and w % 2 == 0:
            h_top = 2 * h // 3          # 顶部大图高度
            h_bottom = h // 3           # 底部小图高度
            w_half = w // 2             # 底部每个小图宽度

            # 1. 提取三个原始区域（不硬编码任何像素值）
            top_img = quad_image[0:h_top, 0:w]               # 顶部大图
            bottom_left = quad_image[h_top:h, 0:w_half]      # 左下
            bottom_right = quad_image[h_top:h, w_half:w]     # 右下

            # 2. 顶部大图 resize 到底部小图的尺寸（w_half, h_bottom）
            top_resized = cv2.resize(
                top_img, (w_half, h_bottom), interpolation=cv2.INTER_AREA
            )

            # 3. 返回 4 个 cell：top / top2 来自顶部，wrist_L / wrist_R 来自底部
            return {
                "top": top_resized.copy(),      # 1`
                "top2": top_resized.copy(),     # 1` 的副本
                "wrist_L": bottom_left.copy(),         # 2
                "wrist_R": bottom_right.copy(),         # 3
            }

        # ========== 旧布局：2x2 quad（保持兼容） ==========
        if h % 2 or w % 2:
            raise ValueError(
                f"Unexpected image dimensions: {h}x{w}. "
                f"Expected either (H%3==0, W%2==0) for 1+2 stacked layout, "
                f"or (H%2==0, W%2==0) for 2x2 quad layout."
            )
    ...
    @staticmethod
    def _split_quad4_image(quad_image: np.ndarray) -> dict[str, np.ndarray]:
    ...
    def get_observation(self) -> RobotObservation:
        ...
        Returns:
            Dictionary with joint positions, raw gripper values, and camera images.
        """
        url = f"{self.config.http_base_url}/state"
        try:
            response = self._session.get(url, timeout=self.config.timeout)
            ...
            try:
                if "gripper_left" in data and data["gripper_left"] is not None:
                    gripper_left_value = self._extract_gripper_pos(data["gripper_left"])
                    if gripper_left_value is None:
                        obs[f"{self.config.joint_names[14]}.pos"] = float(self.config.default_gripper_pos)
                    else:
                        obs[f"{self.config.joint_names[14]}.pos"] = float(gripper_left_value)
                else:
                    obs[f"{self.config.joint_names[14]}.pos"] = float(self.config.default_gripper_pos)

                if "gripper_right" in data and data["gripper_right"] is not None:
                    gripper_right_value = self._extract_gripper_pos(data["gripper_right"])
                    if gripper_right_value is None:
                        obs[f"{self.config.joint_names[15]}.pos"] = float(self.config.default_gripper_pos)
                    else:
                        obs[f"{self.config.joint_names[15]}.pos"] = float(gripper_right_value)
                else:
                    obs[f"{self.config.joint_names[15]}.pos"] = float(self.config.default_gripper_pos)
    ...
    def send_action(self, action: RobotAction) -> RobotAction:
        ...
        Args:
            action: Dictionary with 14 arm targets and two raw gripper targets

        Returns:
            The action actually sent; only arm targets may be clipped
        """
    ...
    def send_action_chunk(self, actions: list[RobotAction]) -> list[RobotAction]:
        ...
        :meth:`send_action`. Every action is put through the same arm-joint
        safety clipping / max-relative-target processing (applied sequentially
        so per-step motion limiting chains through the chunk). Gripper values
        pass through unchanged before all payloads are POSTed together to
        ``config.action_chunk_path``.

        The server must accept a body of the form::

            {"actions": [
                {"jointcmd_left": [7 rad], "jointcmd_right": [7 rad],
                 "gripper_left": raw, "gripper_right": raw},
                ...
            ]}
    ...
    def _prepare_action(self, action: RobotAction) -> tuple[dict, RobotAction]:
        """Build one HTTP payload while preserving raw gripper values.
        ...
        arm_joints_rad = arm_joints_deg

        # Gripper values use their original representation end-to-end.
        left_gripper_value = action[f"{self.config.joint_names[14]}.pos"]
        right_gripper_value = action[f"{self.config.joint_names[15]}.pos"]
        ...
        payload = {
            ...
            "gripper_left": float(left_gripper_value),  # 单个浮点数，不是数组
            "gripper_right": float(right_gripper_value),
        }

        # Update last sent position (only arm joints for relative target limiting)
        self._last_sent_pos = arm_joints_deg.tolist()

        # Build the result with processed arm joints and untouched grippers.
        result: RobotAction = {}
        for i in range(14):
            result[f"{self.config.joint_names[i]}.pos"] = float(arm_joints_deg[i])
        result[f"{self.config.joint_names[14]}.pos"] = float(left_gripper_value)
        result[f"{self.config.joint_names[15]}.pos"] = float(right_gripper_value)
