...
class MarvainM6HttpRobotConfig(RobotConfig):
    ...
    SDK. This config class defines the HTTP endpoint and robot parameters.
    Gripper feedback and command values retain their original representation
    across the HTTP driver boundary.

    HTTP API Structure:
    - GET /state returns:
      - joint_states.positions: [14 arm joints in radians]
      - gripper_left: [raw gripper feedback position]
      - gripper_right: [raw gripper feedback position]
    ...
    Attributes:
        ...
        default_gripper_pos: Raw default gripper position when HTTP doesn't provide one
    ...
    # Used when the HTTP API does not return a valid gripper value.
    default_gripper_pos: float = 0.0  # Raw default gripper position
